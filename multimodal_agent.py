...
class MultiModalAgent():
    ...
    def execute_user_ask(self, user_input=None, chat_history=None, base64image=None, temperature=0.7, filters=None, model=None, json_response=False, reasoning=None, tools=None,
                         tool_call_info_callback=None) -> tuple:
        ...
        while len(response) > 0 and any(single_response.get("type") == "toolcall" for single_response in response):
            for single_response in response:
                if single_response["type"] == "toolcall":
                    tool_response = self.connector.make_tool_calls(single_response["value"], callback=tool_call_info_callback)
